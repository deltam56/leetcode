	  ...
	  public:
		  int robotSim(vector<int>& commands, vector<vector<int>>& obstacles) {
			  ...
			  for(auto& obs : obstacles){
				  obsSet.insert((long long)(obs[0]+30001) * 60002 + (obs[1]+30001));
			  }
			  ...
			  for(int c : commands){
				  ...
				  for(int step = 0; step < c; step++){
					  ...
					  long long key = (long long)(nx+30001) * 60002 + (ny+30001);
					  if(obsSet.count(key)) break;
					  xpos = nx;
					  ypos = ny;
					  res = max(res, xpos*xpos + ypos*ypos);
				  }
			  }
			  return res;
		  }
