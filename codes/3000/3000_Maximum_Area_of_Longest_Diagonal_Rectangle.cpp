...
public:
    int areaOfMaxDiagonal(vector<vector<int>>& dimensions) {
		int max_area = 0;
		long long max_lw = 0;
		for(vector<int> vec : dimensions){
			long long dim = (vec[0]*vec[0])+(vec[1]*vec[1]);
			if(max_lw < dim){
				max_lw = dim;
				max_area = vec[0]*vec[1];
			}else if(max_lw == dim){
				max_area = max(max_area,vec[0]*vec[1]);
			}
		}
		return max_area;
    }
...
int main() {
	...
	ans = 20;
	res = sol.areaOfMaxDiagonal(dimensions);
